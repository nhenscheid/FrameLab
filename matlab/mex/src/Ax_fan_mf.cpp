...
//the user needs to supply "id_X" -- the image index for each projection.
{
    ...
    {   case 0:
            if(GPU==0)
            {Ax_fan_mf_cpu_siddon(X,y,SO,OD,scale,nx,ny,nv,sd_phi,nd,y_det,id_X);}
            else
            {Ax_fan_mf_gpu_siddon(X,y,SO,OD,scale,nx,ny,nv,sd_phi,nd,y_det,id_X,nt);}
        break;
        case 1:
            if(GPU==0)
            {Ax_fan_mf_cpu_new(X,y,SO,OD,scale,nx,ny,nv,sd_phi,nd,y_det,id_X);}
            else
            {Ax_fan_mf_gpu_new(X,y,SO,OD,scale,nx,ny,nv,sd_phi,nd,y_det,id_X,nt);}
        break;
        case 2:
            if(GPU==0)
            {Ax_fan_mf_cpu_new_fb(X,y,SO,OD,scale,nx,ny,nv,sd_phi,nd,y_det,id_X,dy_det);}
            else
            {Ax_fan_mf_gpu_new_fb(X,y,SO,OD,scale,nx,ny,nv,sd_phi,nd,y_det,id_X,nt,dy_det);}
        break;
    }
}
