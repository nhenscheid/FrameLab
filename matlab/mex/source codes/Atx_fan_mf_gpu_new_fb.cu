...
__global__ void Atx_fan_mf_gpu_new_fb_kernel(float *x,float *Y,float SO,float OD,float scale,int nx,int ny,float *sd_phi,int nd,float *y_det,float dy_det,int nt,
int *id_Y,int *Nv,int tmp_size,float *cos_phi,float *sin_phi,float *cos_det,float *sin_det)
...
void Atx_fan_mf_gpu_new_fb(float *x,float *Y,float SO,float OD,float scale,int nx,int ny,float *sd_phi,int nd,float *y_det,float dy_det,int nt,
int *id_Y,int *Nv,int tmp_size,int nv,float *cos_phi,float *sin_phi,float *cos_det,float *sin_det)
...
{   float *x_d,*Y_d,*sd_phi_d,*y_det_d,*cos_phi_d,*sin_phi_d,*cos_det_d,*sin_det_d;
	...
	cudaMalloc(&cos_det_d,(nd+1)*sizeof(float));cudaMemcpy(cos_det_d,cos_det,(nd+1)*sizeof(float),cudaMemcpyHostToDevice);
	cudaMalloc(&sin_det_d,(nd+1)*sizeof(float));cudaMemcpy(sin_det_d,sin_det,(nd+1)*sizeof(float),cudaMemcpyHostToDevice);
	...
	Atx_fan_mf_gpu_new_fb_kernel<<<dimGrid_t, dimBlock>>>(x_d,Y_d,SO,OD,scale,nx,ny,sd_phi_d,nd,y_det_d,dy_det,nt,
	id_Y_d,Nv_d,tmp_size,cos_phi_d,sin_phi_d,cos_det_d,sin_det_d);
